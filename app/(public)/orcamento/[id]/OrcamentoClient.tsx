...
export default function PublicOrderBudgetPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-slate-100 py-10 px-4 flex flex-col items-center font-mono">
      <div className="w-full max-w-xl bg-slate-950 border-2 border-slate-900 p-6 space-y-6 shadow-2xl relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b-2 border-slate-900">
          <div>
            ...
            <p className="text-[9px] text-slate-500 uppercase tracking-widest mt-1">Orçamento & Diagnóstico Técnico</p>
          </div>
          <div className="text-left sm:text-right">
            <span className="text-[9px] text-slate-500 block">Identificação do Chamado</span>
            <span className="text-xs font-bold text-slate-300">#OS-{order.codigo_os || order.id.slice(0, 8)}</span>
          </div>
        </div>
        ...
        {success ? (
          <div className="border border-emerald-500/30 bg-emerald-950/20 p-5 space-y-4">
            ...
            <div className="pt-4 border-t border-slate-900 text-[10px] space-y-2 text-slate-400">
              <p><strong className="text-slate-500">Signatário:</strong> {order.client_signature_name}</p>
              <p><strong className="text-slate-500">Data/Hora do Aceite:</strong> {new Date(order.client_signature_at).toLocaleString('pt-BR')}</p>
              <p><strong className="text-slate-500">Endereço IP de Registro:</strong> {order.client_signature_ip}</p>
            </div>
            
            <div className="pt-4 border-t border-slate-900">
              <p className="text-[9px] text-slate-500 uppercase tracking-wider mb-2">Assinatura Digitalizada:</p>
              <div className="border border-slate-900 bg-slate-950 p-2 h-[80px] flex items-center justify-center">
                <img src={order.client_signature} alt="Assinatura Digital" className="h-full object-contain invert brightness-200" />
              </div>
            </div>
            
            <div className="text-[10px] text-slate-600 leading-relaxed pt-2">
          ...
          <div className="border border-slate-800 bg-slate-950 p-5 text-center space-y-3">
            <Shield className="w-8 h-8 text-slate-500 mx-auto" />
            <h2 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Orçamento não Disponível</h2>
            <p className="text-[10px] text-slate-500 leading-relaxed">
              Este orçamento já foi processado ou encontra-se no status <strong className="text-slate-400 uppercase">[{order.status}]</strong>, não estando mais apto para aprovação pública.
            </p>
          </div>
        ) : (
          <>
            {/* INFORMAÇÕES DO CLIENTE E EQUIPAMENTO */}
            <div className="border border-slate-900 p-4 space-y-4 bg-slate-950">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <span className="text-[9px] text-slate-500 uppercase tracking-wider block">Proprietário</span>
                ...
                <div>
                  <span className="text-[9px] text-slate-500 uppercase tracking-wider block">Equipamento / Modelo</span>
                  <div className="text-xs text-slate-300 mt-1 flex items-center gap-1.5">
                ...
                {serialNumber && serialNumber !== '—' && (
                  <div className="sm:col-span-2">
                    <span className="text-[9px] text-slate-500 uppercase tracking-wider block">Número de Série</span>
                    <span className="text-xs text-slate-300 font-bold block mt-1">{serialNumber}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* DIAGNÓSTICO TÉCNICO */}
            <div className="border border-slate-900 p-4 space-y-2 bg-slate-950">
              <span className="text-[9px] text-slate-500 uppercase tracking-widest block">Laudo Técnico & Parecer do Diagnóstico</span>
            ...
            <div className="space-y-3">
              <span className="text-[9px] text-slate-500 uppercase tracking-widest block">Demonstrativo de Valores</span>
              
              <div className="border border-slate-900 divide-y divide-slate-900 bg-slate-950 text-xs">
                
                {/* Mão de Obra */}
                {parseFloat(order.service_value) > 0 && (
                  <div className="p-3 flex justify-between gap-4">
                    <span className="text-slate-400">Serviços Técnicos e Mão de Obra</span>
                    <span className="text-slate-300 font-bold">R$ {parseFloat(order.service_value).toFixed(2)}</span>
                  </div>
                )}
                ...
                {orderServices.map((item, idx) => (
                  <div key={`serv-${idx}`} className="p-3 flex justify-between gap-4">
                    <span className="text-slate-400">{item.service_name} <span className="text-[9px] text-slate-600 ml-1">x{item.quantidade}</span></span>
                    <span className="text-slate-300 font-bold">R$ {parseFloat(item.subtotal).toFixed(2)}</span>
                  </div>
                ))}
                ...
                {orderItems.map((item, idx) => (
                  <div key={`prod-${idx}`} className="p-3 flex justify-between gap-4">
                    <span className="text-slate-400">{item.product_name} <span className="text-[9px] text-slate-600 ml-1">x{item.quantity}</span></span>
                    <span className="text-slate-300 font-bold">R$ {(item.quantity * item.unit_price).toFixed(2)}</span>
                  </div>
                ))}

                {/* Total */}
                <div className="p-4 bg-slate-950 flex justify-between items-center text-sm">
            ...
            <div className="border border-slate-900 bg-slate-950">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Termos de Autorização & Garantia</span>
                </div>
                <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${isTermsOpen ? 'rotate-180' : ''}`} />
              </button>
              
              {isTermsOpen && (
                <div className="p-4 pt-0 border-t border-slate-900 text-[10px] text-slate-500 leading-relaxed space-y-3 whitespace-pre-line font-mono">
...
O prazo estimado para início da execução dos serviços é de 2 a 5 dias úteis a partir do aceite digital deste termo, condicionado à disponibilidade de peças em estoque.`}
            ...
            <div className="border border-slate-900 p-4 space-y-4 bg-slate-950">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Assinatura Eletrônica do Cliente</span>
              </div>
              
              <div className="space-y-1.5">
                <label htmlFor="signer-name" className="text-[9px] text-slate-500 uppercase tracking-wider block">Nome Completo do Signatário</label>
                <input
                  id="signer-name"
                  type="text"
                  ...
                  className="w-full bg-transparent border-b border-slate-800 focus:border-slate-600 px-2 py-2 text-xs text-slate-300 placeholder:text-slate-700 focus:outline-none transition-colors font-mono"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-[9px] text-slate-500 uppercase tracking-wider block">Escreva sua assinatura digital com o dedo na tela</label>
                <SignaturePad 
