...
export default async function BackofficePage() {
  ...
  if (error) {
    ...
    return (
      <div className="p-8 text-center bg-red-900/20 border border-danger/40 rounded-xl text-danger">
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-small text-text-muted mt-1">Gestão global de todas as empresas cadastradas no SaaS.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-surface-raised border border-border rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-text-muted font-medium">Total de Empresas</h3>
            <div className="p-2 bg-blue-500/10 rounded-lg">
              <BuildingIcon className="w-5 h-5 text-info" />
            </div>
          </div>
        ...
        <div className="bg-surface-raised border border-border rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-text-muted font-medium">Assinaturas Ativas</h3>
            <div className="p-2 bg-brand/10 rounded-lg">
              <Activity className="w-5 h-5 text-brand" />
            </div>
          </div>
          <p className="text-4xl font-bold text-brand">{activeTenants}</p>
        </div>

        <div className="bg-surface-raised border-danger/25 border rounded-xl p-6 shadow-[0_0_15px_rgba(239,68,68,0.1)] relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-danger/10 blur-2xl rounded-full"></div>
          <div className="flex items-center justify-between mb-4 relative z-10">
            <h3 className="text-danger font-medium">Status de Segurança</h3>
            <div className="p-2 bg-danger/20 rounded-lg">
              <ShieldAlert className="w-5 h-5 text-danger" />
            </div>
          </div>
          <p className="text-xl font-bold text-danger relative z-10 uppercase tracking-wider">GOD MODE ON</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm text-text">
            ...
            <tbody className="divide-y divide-border">
              {companies?.map((company) => (
                <tr key={company.id} className="hover:bg-surface-overlay/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-white">{company.name}</td>
                  <td className="px-6 py-4 text-text-muted">{company.subdomain || '-'}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3 text-xs">
                      <span className="flex items-center text-text-muted" title="Ordens de Serviço">
                        <Activity className="w-3 h-3 mr-1" />
                        {company.service_orders?.[0]?.count || 0}
                      </span>
                      <span className="flex items-center text-text-muted" title="Usuários Ativos">
